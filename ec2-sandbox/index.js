...
function resolveBin(name, fallback) {
  try {
    return execSync(`which ${name}`, { encoding: "utf8" }).trim().split("\n")[0].trim();
  } catch {
    return fallback;
  }
}
...
async function cloneRepo(repoUrl, clonePath) {
  await execFileAsync("rm", ["-rf", clonePath], { timeout: 10_000 }).catch(() => {});
...
async function buildImage(imageName, clonePath) {
  await docker(["build", "-t", imageName, clonePath], "build", 600_000);
}

async function startApp(imageName, appContainer, mongoContainer, dbName) {
...
async function cleanup(meta) {
  ...
  await execFileAsync("rm", ["-rf", meta.clonePath], { timeout: 10_000 }).catch(() => {});
}
...
app.post("/launch", async (req, res) => {
  ...
  console.log(`\n${"─".repeat(60)}`);
  console.log(`[launch] ${meta.sandboxId}`);
  console.log(`[launch] ${meta.repoUrl}`);
  ...
  try {
    ...
    await buildImage(meta.imageName, meta.clonePath);
    const port = await startApp(meta.imageName, meta.appContainer, meta.mongoContainer, meta.dbName);
...
app.listen(PORT, () => console.log(`[sandbox-engine] Listening on :${PORT}`));
